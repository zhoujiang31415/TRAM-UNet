...
class BasicTransBlock(nn.Module):
    ...
    def __init__(self, in_ch, heads, dim_head, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, peg_pos=True):
        super().__init__()
        self.bn1 = nn.BatchNorm2d(in_ch)

        self.attn = LinearAttention(in_ch, heads=heads, dim_head=in_ch//heads, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, peg_pos=peg_pos)
...
class CosBasicTransDecoderBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, heads, dim_head, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp',
        ...
        self.attn = CosLinearAttentionDecoder(in_ch, out_ch, heads=heads, dim_head=dim_head, attn_drop=attn_drop,
...
class BasicTransDecoderBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, heads, dim_head, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, peg_pos=True):
        super().__init__()
        ...
        self.attn = LinearAttentionDecoder(in_ch, out_ch, heads=heads, dim_head=dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, peg_pos=peg_pos)
...
class LinearAttention(nn.Module):
    ...
    def __init__(self, dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, peg_pos=True):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # if self.peg_pos:
        #     conditional_position_bias = self.peg(q_k_attn)
        #     x = q_k_attn
        #     q_k_attn = x + conditional_position_bias
...
class CosLinearAttentionDecoder(nn.Module):
    # in_dim=512, out_dim=256, head=8, dim_head=64
    def __init__(self, in_dim, out_dim, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16,
        ...
        self.inner_dim = dim_head * heads   # inner_dim=512
        self.heads = heads
        self.scale = dim_head ** (-0.5)
        self.dim_head = dim_head
        self.reduce_size = reduce_size
        ...
        self.to_qk = depthwise_separable_conv(in_dim, self.inner_dim * 2)
        self.to_v = depthwise_separable_conv(out_dim, self.inner_dim)
    ...
    def forward(self, v, x):
        B, C, H, W = x.shape  # low-res feature shape       512*8*8
        BH, CH, HH, WH = v.shape  # high-res feature shape  256*16*16

        q, k = self.to_qk(x).chunk(2, dim=1)  # B, inner_dim, H, W, kv=512*8*8
        v = self.to_v(v)  # BH, inner_dim, HH, WH, q=512*16*16

        if self.projection == 'interp' and H != HH:
            q, k = map(lambda t: F.interpolate(t, size=HH, mode='bilinear', align_corners=True), (q, k))

        elif self.projection == 'maxpool' and H != HH:
            q, k = map(lambda t: F.adaptive_max_pool2d(t, output_size=HH), (q, k))

        v = rearrange(v, 'b (dim_head heads) h w -> b heads (h w) dim_head', dim_head=self.dim_head, heads=self.heads,
                      h=HH, w=WH)
        q, k = map(lambda t: rearrange(t, 'b (dim_head heads) h w -> b heads (h w) dim_head', dim_head=self.dim_head,
                                       heads=self.heads, h=HH, w=HH), (q, k))

        q_k_attn = torch.einsum('bhid,bhjd->bhij', q, k)  # 2*(64*8)*(16*16) -> 2*8*(16*16)*64, 2*8*(16*16)*(16*16)
        if self.rel_pos:
        ...
        out = torch.einsum('bhij,bhjd->bhid', q_k_attn, v)  # q_k_attn, 2*8*(16*16)*(16*16);v, 2*8*(16*16)*64
...
class LinearAttentionDecoder(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, heads=8, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, peg_pos=True):
        super().__init__()
    ...
    def forward(self, q, x):
        B, C, H, W = x.shape # low-res feature shape 512*8*8
        BH, CH, HH, WH = q.shape # high-res feature shape 256*16*16
        ...
        out = torch.einsum('bhij,bhjd->bhid', q_k_attn, v)  # 2*8*(16*16)*64
...
class ConditionalPositionalEncodings(nn.Module):
    def __init__(self, in_ch, out_ch, stride=1, kernel_size=3, padding=1, bias=False):
        super().__init__()

        self.peg = depthwise_separable_conv(in_ch, out_ch, stride=stride,
...
class my_down_block_trans(nn.Module):
    def __init__(self, in_ch, num_block, heads=4, dim_head=64, attn_drop=0.,
                 proj_drop=0., reduce_size=8, projection='interp', rel_pos=True, peg_pos=True):
        ...
        for i in range(num_block):
            block_list.append(
                attn_block(in_ch, heads, dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size,
                           projection=projection, rel_pos=rel_pos, peg_pos=peg_pos))
...
class cos_up_block_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0.,
        ...
        self.attn_decoder = CosBasicTransDecoderBlock(in_ch, out_ch, heads=heads, dim_head=dim_head, attn_drop=attn_drop,
                                                      proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                                      rel_pos=rel_pos)
...
class up_block_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True, peg_pos=True):
        super().__init__()
 
        self.attn_decoder = BasicTransDecoderBlock(in_ch, out_ch, heads=heads, dim_head=dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, peg_pos=peg_pos)
        ...
        for i in range(num_block):
            block_list.append(attn_block(out_ch, heads, dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos, peg_pos=peg_pos))
