...
class UTNet(nn.Module):
    ...
    def __init__(self, n_channels, base_chan, n_classes=1, reduce_size=8, block_list='234', num_blocks=[1, 2, 4],
        ...
        if '1' in block_list:
            self.down1 = down_block_trans(base_chan, 2 * base_chan, num_block=num_blocks[-4], bottleneck=bottleneck,
                                             ...
                                             dim_head=2 * base_chan // num_heads[-4], attn_drop=attn_drop,
        ...
        if '2' in block_list:
            self.down2 = down_block_trans(2 * base_chan, 4 * base_chan, num_block=num_blocks[-3],
                                             bottleneck=bottleneck, maxpool=maxpool, heads=num_heads[-3],
                                             dim_head=4 * base_chan // num_heads[-3], attn_drop=attn_drop,
                                             proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                             rel_pos=rel_pos)
        ...
        if '3' in block_list:
            self.down3 = down_block_trans(4 * base_chan, 8 * base_chan, num_block=num_blocks[-2],
                                             bottleneck=bottleneck, maxpool=maxpool, heads=num_heads[-2],
                                             dim_head=8 * base_chan // num_heads[-2], attn_drop=attn_drop,
                                             proj_drop=proj_drop, reduce_size=reduce_size, projection=projection,
                                             rel_pos=rel_pos)
    ...
    def forward(self, x):
        ...
        if self.aux_loss:
            out = self.up1(x5, x4)
            out1 = F.interpolate(self.out1(out), size=x.shape[-2:])

            out = self.up2(out, x3)
            out2 = F.interpolate(self.out2(out), size=x.shape[-2:])

            out = self.up3(out, x2)
            out3 = F.interpolate(self.out3(out), size=x.shape[-2:])
